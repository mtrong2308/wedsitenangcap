    ...
    <section id="apartments" className="py-24 px-6 bg-white">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          <div>
            <h2 className="text-4xl font-bold text-slate-900 mb-4">Dự án đang nghiên cứu</h2>
            <p className="text-slate-500">Các công nghệ và giải pháp đột phá chúng tôi đang tập trung phát triển.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {filtered.map((proj, i) => (
            <div key={i} className="group cursor-pointer">
              <div className="relative rounded-3xl overflow-hidden mb-4 shadow-md">
                <img 
                  src={proj.img} 
                  alt={proj.title} 
              ...
              <h3 className="text-lg font-bold text-slate-900 group-hover:text-blue-600 transition-colors">{proj.title}</h3>
              <p className="text-blue-600 font-medium text-sm">{proj.type}</p>
            </div>
          ))}
        </div>
      </div>
